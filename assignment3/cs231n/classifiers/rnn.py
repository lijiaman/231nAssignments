...
class CaptioningRNN(object):
  ...
  def sample(self, features, max_length=30):
    ...
    for i in xrange(max_length):
        word_embed, embed_cache = word_embedding_forward(capt, W_embed)
        
        if self.cell_type == "rnn":
            h, cache_rnn = rnn_step_forward(np.squeeze(word_embed), prev_h, Wx, Wh, b)
        elif self.cell_type == "lstm":
            h, c, cache_rnn = lstm_step_forward(np.squeeze(word_embed), prev_h, prev_c, Wx, Wh, b)
            
        scores, scores_cache = temporal_affine_forward(h[:, np.newaxis, :], W_vocab, b_vocab)
